...
class Element(models.Model):
    ...
    element_type = models.CharField(
        'Тип', max_length=30, choices=[(tag.name, tag.value) for tag in ElementType]
    )
    ...
    truth_table = JSONField('Временная дискретизация таблицы истинности', validators=[
        JSONArrayFieldValidator(field_validator=bin_validator, dimension=3)
    ])
