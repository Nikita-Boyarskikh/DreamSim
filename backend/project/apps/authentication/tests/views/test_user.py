...
class TestUserViewSet(ViewTestCase):
    ...
    def test_login(self, client, simple_user, simple_user_password):
        client.logout()
        data = {'username': simple_user.username, 'password': simple_user_password}
        response = client.post(self.get_url() + 'login/', data=data)
        assert_response(response, 200, self.serialize(simple_user))
    ...
    def test_login_wrong_credentials(self, client, simple_user, simple_user_password):
        password = 'wrong_password'
        assert password != simple_user_password
        data = {'username': simple_user.username, 'password': password}
    ...
    def test_login_already_authorized(self, client, simple_user):
        data = {}
        assert simple_user
        response = client.post(self.get_url() + 'login/', data=data)
        assert_response(response, 200, self.serialize(simple_user))
    ...
    def test_login_user_does_not_exists_already_authorized(self, client, simple_user):
        assert simple_user

        data = {'username': 'does_not_exists', 'password': 'simple_password'}
        response = client.post(self.get_url() + 'login/', data=data)
    ...
    def test_login_wrong_credentials_already_authorized(self, client, simple_user, simple_user_password):
        password = 'wrong_password'
        assert password != simple_user_password
        assert simple_user
    ...
    def test_logout_already_authorized(self, client, simple_user):
        assert simple_user
        response = client.post(self.get_url() + 'logout/')
        assert_response(response, 200)
        assert not self.is_logged_in(response)
